...
export function findCommonKey(arg1:{},arg2:{}):string[]|string{
    if(typeof(arg1)!=="object"||typeof(arg2)!=="object" ||arg1===null||arg2===null ||Array.isArray(arg1)||Array.isArray(arg2)) return "Invalid"
    let result:string[]=[]
    let key1=Object.keys(arg1)
    let key2=Object.keys(arg2)
    if(key1.length===0||key2.length==0) return "Invalid"
    for(let key of key1){
        if(key2.includes(key)){
            result.push(key)
        }
    }
    if(result.length===0) return "No Common Keys"
    else return result
}
